...
class AgentController:
    ...
    def _is_small_talk(query: str) -> bool:
        ...
        return any(phrase in sanitized for phrase in trivial_phrases)

    def decide(self, query: str) -> AgentDecision:
        if not query.strip():
            return AgentDecision(require_retrieval=False, reason="Empty query")
        if self._is_small_talk(query):
            return AgentDecision(require_retrieval=False, reason="Small talk")
        return AgentDecision(require_retrieval=True, reason="Information-seeking query")
